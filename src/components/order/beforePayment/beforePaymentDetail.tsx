...
const BeforePaymentDetailBlock = styled(Responsive)``;

type beforePaymentDetailProps = {
  beforePaymentDetail?: { [key: string]: any };
};

// const BeforePaymentDetail = ({ data }: beforePaymentDetailProps) => {
const BeforePaymentDetail = ({
  beforePaymentDetail,
}: beforePaymentDetailProps) => {
  const data = testVendorOrderData[0];
  const item = data?.dealerOrderItem;
  return (
    <Fragment>
      <BeforePaymentDetailBlock>
        <PageHeader
          breadCrumb={
            <BreadCrumb
              indicator={[
...
export default BeforePaymentDetail;
