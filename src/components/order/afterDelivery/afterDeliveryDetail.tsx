...
const AfterDeliveryDetailBlock = styled(Responsive)``;

type afterDeliveryDetailProps = {
  afterDelivery?: { [key: string]: any };
};

// const AfterDeliveryDetail = ({ data }: afterDeliveryDetailProps) => {
const AfterDeliveryDetail = ({ afterDelivery }: afterDeliveryDetailProps) => {
  const data = testVendorOrderData[0];
  const item = data?.dealerOrderItem;
  return (
    <Fragment>
      <AfterDeliveryDetailBlock>
        <PageHeader
          breadCrumb={
            <BreadCrumb
              indicator={[
...
export default AfterDeliveryDetail;
